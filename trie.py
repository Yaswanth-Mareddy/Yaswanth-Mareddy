...
def main():
    ...
    ### WRITE YOUR CODE HERE###
    #print(text)
    lst_misspelled_words=[]
    text = open(sys.argv[2], "r")
    for line in text:
        for word in line.split():
            word = word.lower().strip(',').strip('.')
            new_trie=trie
            #print("entered",new_trie.__contains__(word))
            if not new_trie.__contains__(word):
                   lst_misspelled_words.append(word)
    for word in lst_misspelled_words:
        print(word)       
main()
