...
class Heap:
    ...
    def __largestChild(self, index, lastIndex):
        ...
        while(index<=lastIndex):
            if(self.data[index]>self.data[largest_index]):
                largest_index=index
            index+=1
    ...
    def __siftDownFromTo(self, fromIndex, last):
        ...
        '''
        parent_index=fromIndex
        if(parent_index<=last):
            large_index=self.__largestChild(parent_index,last)
            if(large_index==None):
                return None
            if(self.data[parent_index]<=self.data[large_index]):
                temp=self.data[parent_index]
                self.data[parent_index]=self.data[large_index]
                self.data[large_index]=temp
